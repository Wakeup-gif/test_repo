...
async function bootPage(tabId) {
  ...
  return responseForProbe(classification, probe, {
    source,
    expectedB1Degraded: classification === PROBE_RESULTS.DEGRADED_SAME_BUILD && probe.runtimeSnapshot?.reason === 'coordination-not-implemented-b1'
  });
}
...
async function setPageEnabled(tabId, enabled) {
  ...
  if (!Number.isInteger(tabId)) return { ok: true, enabled: Boolean(enabled) };

  if (enabled) return bootPage(tabId);
  ...
  try {
    const result = await chrome.scripting.executeScript({
      ...
      func: async () => {
        ...
        if (!runtime?.teardown) return { ok: true, state: 'UNINITIALIZED', reason: 'user-disabled' };
        const state = await runtime.teardown('user-disabled');
        return { ok: true, ...state };
      }
    });
    return result?.[0]?.result || { ok: true, enabled: false };
  } catch (error) {
...
async function revalidatePage(tabId) {
  ...
  if (![PROBE_RESULTS.HEALTHY_SAME_BUILD, PROBE_RESULTS.DEGRADED_SAME_BUILD, PROBE_RESULTS.RECOVERING_SAME_BUILD].includes(classification)) {
    return responseForProbe(classification, probe);
  }
  ...
  const result = await chrome.scripting.executeScript({
    target: { tabId },
    world: 'MAIN',
    func: async () => window.__squareCoilCompanionRuntime?.revalidate?.() || null
  });
  return { ok: true, classification, health: result?.[0]?.result || null };
}

...
function sourceIdentity() {
  ...
  const sourceDirty=gitOutput(['status','--porcelain','--untracked-files=all'],'inspect source-tree dirtiness').length>0;
...
function bundle(entryId){
  ...
  function visit(id){if(modules.has(id))return;const full=path.join(root,...id.split('/'));const source=fs.readFileSync(full,'utf8');modules.set(id,source);deps[id]=Object.create(null);const requirePattern=/require\(\s*(['"])([^'"]+)\1\s*\)/g;let match;while((match=requirePattern.exec(source))){const request=match[2];if(!request.startsWith('.'))throw new Error(`Only relative requires are supported in browser bundles: ${id} -> ${request}`);const resolved=resolveDependency(id,request);deps[id][request]=resolved;visit(resolved);}}
  visit(entryId);

...
| Probe-backed menu, Leads, and Install Calendar repairs | B5-C light | B5-C Logic/evidence, v2.3.3 read-only probe | Core Sleek Dark adapter/on with Sleek Dark | existing SquareCoil host only | CSS-only presentation after bounded pathname classification | Accepted at `aeac7fe`; 4 B5-C A4 fixtures PASS in Chrome/Edge; native presentation restores exactly |
...
| Exact dual-browser release candidate | B6 | build/package identity, A4 harness, B6 evidence | Candidate gate; no promotion | unchanged accepted permissions | packaging/verification only | Accepted at `aeac7fe`; 482 aggregate + 13 prototype tests, Chrome/Edge clean 27/27 and upgrade 2/2, CI success |
...
| Route-bounded vendor/editor theme parity and zero-history navigation | B5-D | `logic/B5C-PROBE-THEME-DELTA.md`, v2.3.3 evidence, B5-D evidence | Core Sleek Dark adapter/on with Sleek Dark | existing SquareCoil host only | presentation/navigation only | Accepted at `4686ab0`; five B5-D A4 fixtures PASS in Chrome/Edge; exact native/accessibility teardown |
...
Every accepted B5-B optional feature has a stable ID/version, category, route list, permissions, dependencies/conflicts, storage/migration, activation/deactivation/teardown/recovery, health, privacy/retention, automated coverage, and installed-browser evidence. B5-C remains the bounded light probe delta; B5-D separately completes the accepted route/vendor/editor presentation scope and zero-history navigation. See `implementation/B5D-THEME-UI-EVIDENCE.md`. Candidate acceptance is not production promotion or publication.

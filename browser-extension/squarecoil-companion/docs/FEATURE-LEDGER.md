...
| Full probe-informed route/vendor theme parity | future B5-D | `logic/B5C-PROBE-THEME-DELTA.md`, v2.3.3 probe | Optional/deferred | no new permission without separate proof | presentation-only | CKEditor and broader vendor/route adapters explicitly deferred; separately gated |
...
Every accepted B5-B optional feature has a stable ID/version, category, route list, permissions, dependencies/conflicts, storage/migration, activation/deactivation/teardown/recovery, health, privacy/retention, automated coverage, and installed-browser evidence. B5-C remains the bounded CSS-only probe delta; B6 reconciles the full accepted scope against one exact clean package. See `implementation/B6-RELEASE-CANDIDATE-EVIDENCE.md`. Candidate acceptance is not production promotion or publication.

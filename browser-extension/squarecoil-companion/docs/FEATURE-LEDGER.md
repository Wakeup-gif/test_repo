...
| Cinematic/Bing presentation pack | B5-B | isolated reskin audit/lab | Optional/off | optional network host permission if proven | presentation/network read only | Current gate; privacy/cache/stale-result and source-policy audit required before activation |
| Design Dashboard presentation profile | B5-B | historical UI lineage | Optional/off | route host only | presentation-only | Current gate; exact route, selector, lifecycle, and teardown proof required before activation |

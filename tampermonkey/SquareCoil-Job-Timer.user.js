...
// @version      1.1.0
// @description  Consolidated standalone SquareCoil job timer with recent tabs, drag reorder, auto-open context switches, history, and cross-tab sync.
...
(() => {
  ...
  const MAX_SESSIONS_PER_CONTEXT = 180;
  ...
  function normalize(raw) {
    ...
    if (!Array.isArray(out.ui.tabOrder)) out.ui.tabOrder = [];

    out.ui.hiddenKeys = [...new Set(out.ui.hiddenKeys.filter(Boolean))];
    out.ui.tabOrder = [...new Set(out.ui.tabOrder.filter(Boolean))];
    ...
    if (out.active?.key && !out.contexts[out.active.key]) out.active = null;
    if (out.pending?.key && !out.contexts[out.pending.key]) out.pending = null;
    if (out.ui.selectedKey && !out.contexts[out.ui.selectedKey]) out.ui.selectedKey = null;
  ...
  function save(reason, shouldRender = true) {
    const stored = load();
  ...
  function makeContext(projectId, label) {
    ...
    if (isProdGeneral) {
      return {
        ...
        projectId: '0',
        shortLabel: 'General',
        label: clean || 'Production (General)'
      };
    }
    ...
    if (resolvedPid) {
      return {
        ...
        shortLabel: resolvedPid,
        label: clean || `Job ${resolvedPid}`
      };
    }

    if (!clean) return null;
    return {
      key: `general:${slug(clean)}`,
      type: 'general',
      projectId: '0',
      shortLabel: clean.slice(0, 16),
      label: clean
    };
  }
  ...
  function fromDom() {
    ...
    if (hasIn && hasOut && visible('#clockin') && !visible('#clockout')) {
      return { kind: 'out' };
    }
  ...
  function ensureContext(context) {
    ...
    if (!state.contexts[context.key]) {
      state.contexts[context.key] = {
        ...context,
        accumulatedMs: 0,
        sessions: [],
        cycleId: id('cycle'),
      ...
      Object.assign(state.contexts[context.key], {
        type: context.type,
        projectId: context.projectId,
        label: context.label || state.contexts[context.key].label,
        shortLabel: context.shortLabel || state.contexts[context.key].shortLabel,
        lastTouchedAt: now
      });
    }

    showContext(context.key);
    return state.contexts[context.key];
  }
  ...
  function pauseActive(endAt, reason, certainty = 'exact') {
    ...
    if (context.sessions.length > MAX_SESSIONS_PER_CONTEXT) {
      context.sessions = context.sessions.slice(-MAX_SESSIONS_PER_CONTEXT);
    }
  ...
  function observe(observation, source = 'dom', exact = false) {
    pull();
    const now = Date.now();
    if (!observation || observation.kind === 'unknown') return;

    if (observation.kind === 'out' || observation.kind === 'idle') {
      if (state.active) {
      ...
      save(observation.kind === 'out' ? 'clocked-out' : 'idle-clock-context');
      return;
    }
    ...
    const previousClockKey = state.active?.key || state.pending?.key || state.meta.observedClockKey || null;
    const switchedContext = !!previousClockKey && previousClockKey !== incoming.key;
    const context = ensureContext(incoming);
    ...
    state.ui.selectedKey = incoming.key;
    showContext(incoming.key);
    if (switchedContext) state.ui.collapsed = false;
    ...
    if (remembered) {
      state.pending = {
        ...
        detectedAt: now,
        source,
        previousMs: context.accumulatedMs
      };
      save(switchedContext ? 'switch-ask-resume' : 'ask-resume');
      return;
    }
    ...
    save(switchedContext ? 'switch-start-context' : 'start-new-context');
  }
  ...
  function level(ms) {
    const mins = ms / 60000;
    const yellow = Math.max(1, Number(state.settings.yellow) || 60);
    const orange = Math.max(yellow, Number(state.settings.orange) || 120);
    const red = Math.max(orange, Number(state.settings.red) || 240);

    return mins >= red ? 3 : mins >= orange ? 2 : mins >= yellow ? 1 : 0;
  }

  function orderedVisibleContexts() {
    const hidden = new Set(state.ui.hiddenKeys);
    const maxJobs = Math.max(1, Math.min(5, Number(state.settings.maxJobTabs) || 5));
  ...
  function displayKey() {
    if (
      ...
      state.contexts[state.ui.selectedKey] &&
      !state.ui.hiddenKeys.includes(state.ui.selectedKey)
    ) return state.ui.selectedKey;
  ...
  function statusFor(key) {
    ...
    if (reason === 'clocked-out-completely' || reason === 'clocked-out') return 'Clocked out';
    return 'Paused';
  }
  ...
  function historyRows() {
    ...
    for (const context of Object.values(state.contexts)) {
      for (const session of context.sessions) rows.push({ context, session });
    }

    return rows
      .sort((a, b) => (b.session.endAt || 0) - (a.session.endAt || 0))
      .slice(0, MAX_HISTORY);
  }
  ...
  function renderTabs(now) {
    return orderedVisibleContexts().map(context => {
      ...
      const ms = elapsed(key, now);
      const selected = key === displayKey();
      const protectedTab = state.active?.key === key || state.pending?.key === key;
      const close = protectedTab ? '' : `<button type="button" class="jt-x" data-action="hide-tab" data-key="${esc(key)}" aria-label="Hide ${esc(context.shortLabel)}" title="Hide tab"></button>`;

      return `<div class="jt-tab jt-tab-level-${level(ms)}${selected ? ' jt-selected' : ''}${protectedTab ? ' jt-clock-active' : ''}" draggable="true" role="tab" aria-selected="${selected ? 'true' : 'false'}" data-action="select-tab" data-key="${esc(key)}" title="${esc(context.label)} • Click to view • Double-click to open • Drag to reorder"><i class="jt-tab-dot"></i><span>${esc(context.shortLabel)}</span><b class="jt-tab-time" data-tab-time="${esc(key)}">${esc(fmt(ms))}</b>${close}</div>`;
    }).join('');
  }

  function renderSettings() {
    const hiddenContexts = state.ui.hiddenKeys
      .map(key => state.contexts[key])
      .filter(Boolean)
      .sort((a, b) => (b.lastTouchedAt || 0) - (a.lastTouchedAt || 0));

    const hiddenRows = hiddenContexts.length
      ? hiddenContexts.map(context => `<div class="jt-row"><div><b>${esc(context.shortLabel)} · ${esc(context.label)}</b><small>Saved ${esc(fmt(elapsed(context.key), true))}</small></div><button class="jt-btn" data-action="restore" data-key="${esc(context.key)}">Show</button></div>`).join('')
      : '<small class="jt-archive-empty">No hidden tabs.</small>';

    const history = historyRows();
    const historyHtml = history.length
      ? history.map(({ context, session }) => {
          const when = new Date(session.endAt || Date.now()).toLocaleString([], {
  ...
  function render() {
    ...
    root.className = `jt-level-${level(ms)}${running ? ' jt-running' : ''}${state.ui.collapsed ? ' jt-collapsed' : ''}${settingsOpen ? ' jt-settings-open' : ''}`;
    ...
    const contextLabel = context
      ? (context.type === 'job' ? context.projectId : context.shortLabel)
      : 'No timer';
  ...
  function onClick(event) {
    ...
    if (action === 'select-tab') {
      if (Date.now() - lastDragEndAt < DRAG_CLICK_GUARD_MS) return;
    ...
    if (action === 'clear-all') {
      ...
      if (activeKey && keep[activeKey]) {
        Object.assign(state.active, {
          startedAt: Date.now(),
          sessionId: id('session'),
          cycleId: keep[activeKey].cycleId,
          lastVerifiedAt: Date.now()
        });
      } else {
        state.active = null;
      ...
      if (pendingKey && keep[pendingKey]) {
        Object.assign(state.pending, { previousMs: 0, detectedAt: Date.now() });
      } else {
        state.pending = null;
      }
  ...
  function onDragStart(event) {
    const tab = tabFromEvent(event);
    if (!tab || event.target.closest('.jt-x')) {
  ...
  function saveDomTabOrder(bar) {
    pull();
    const visibleOrder = [...bar.querySelectorAll('.jt-tab[data-key]')]
      .map(tab => tab.dataset.key)
      .filter(Boolean);
    const remaining = state.ui.tabOrder.filter(key => !visibleOrder.includes(key));
    state.ui.tabOrder = [...visibleOrder, ...remaining];
    save('tab-reorder');
  }
  ...
  function tick() {
    ...
    root.querySelectorAll('[data-role="time"],[data-role="compact-time"]').forEach(el => {
      el.textContent = fmt(selectedMs);
    });

    root.querySelectorAll('[data-tab-time]').forEach(el => {
      const key = el.dataset.tabTime;
      const ms = elapsed(key, now);
  ...
  async function syncServer(source = 'server', exact = false) {
    ...
    try {
      ...
      // Read-only sync failures never modify SquareCoil's clock state.
    } finally {
      syncing = false;
    }
  }
  ...
  function syncDom(source = 'dom') {
    ...
    if (observation.kind !== 'unknown') observe(observation, source, false);
  }
  ...
  function passiveClockClickFallback() {
    document.addEventListener('click', event => {
      ...
      const exact = [
        ...
        'time-clock-clock-in-to-project-department-2',
        'confirm-clockin',
        'clock-into-project-from-calendar'
      ].includes(button.id);
  ...
  function watchClockDom() {
    ...
    observer.observe(document.body, {
      ...
      characterData: true,
      attributes: true,
      attributeFilter: ['style', 'class', 'data-time']
    });
  }
  ...
  function debugApi() {
    window.__squareCoilJobTimerDebug = () => ({
      ...
      visibleKeys: orderedVisibleContexts().map(context => context.key),
  ...
  function init() {
    ...
    setInterval(() => {
      if (document.visibilityState === 'visible') syncServer('heartbeat', false);
    }, HEARTBEAT_MS);

    window.addEventListener('focus', () => syncServer('focus', false));
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') syncServer('visible', false);
    });
  ...
  function injectStyle() {
    ...
    style.id = 'ussign-job-timer-v110-style';
    style.textContent = `
#${ROOT_ID}{
  --a:#7baaf2;
  --as:rgba(123,170,242,.14);
  --jt-glass:var(--us-glass,rgba(18,18,21,.62));
  --jt-glass-soft:var(--us-glass-soft,rgba(18,18,21,.55));
  --jt-glass-strong:var(--us-glass-strong,rgba(18,18,21,.72));
  ...
  padding-top:39px;
  color:var(--us-text,#eef1f5);
  font:13px/1.35 var(--us-font,Manrope,"Segoe UI",Arial,sans-serif);
  isolation:isolate;
}
...
#${ROOT_ID} .jt-shell{
  ...
  border-radius:14px;
  background:linear-gradient(180deg,rgba(255,255,255,.022),rgba(255,255,255,.003)),var(--jt-glass);
  box-shadow:0 14px 38px rgba(0,0,0,.20),inset 0 1px 0 rgba(255,255,255,.025);
  -webkit-backdrop-filter:blur(18px) saturate(114%) brightness(91%);
  backdrop-filter:blur(18px) saturate(114%) brightness(91%);
}
#${ROOT_ID}.jt-collapsed .jt-shell{
  border-radius:18px;
  background:linear-gradient(180deg,rgba(255,255,255,.028),rgba(255,255,255,.004)),rgba(15,16,20,.68);
  box-shadow:0 14px 34px rgba(0,0,0,.20),inset 0 1px 0 rgba(255,255,255,.03);
  -webkit-backdrop-filter:blur(20px) saturate(116%) brightness(92%);
  backdrop-filter:blur(20px) saturate(116%) brightness(92%);
}
#${ROOT_ID} header{
  min-height:52px;
  display:grid;
...
#${ROOT_ID} .jt-brand{display:flex;align-items:center;gap:9px;color:rgba(235,239,244,.88);font-size:12.5px;font-weight:650;white-space:nowrap}
#${ROOT_ID} .jt-brand>i{position:relative;width:14px;height:14px;flex:0 0 14px;border:2px solid rgba(113,177,229,.90);border-radius:50%}
#${ROOT_ID} .jt-brand>i::before{content:"";position:absolute;left:4px;top:-5px;width:3px;height:3px;border-radius:1px;background:rgba(113,177,229,.90)}
#${ROOT_ID} .jt-brand>i::after{content:"";position:absolute;left:5px;top:2px;width:1px;height:5px;background:rgba(113,177,229,.90);transform-origin:bottom center;transform:rotate(18deg)}
#${ROOT_ID} .jt-head-context{display:none;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:rgba(242,245,248,.90);font-size:12px;font-weight:650}
#${ROOT_ID}.jt-collapsed .jt-head-context{display:block;padding-left:12px;border-left:1px solid rgba(255,255,255,.07)}
#${ROOT_ID} header>[data-role="compact-time"]{justify-self:end;color:rgba(245,247,249,.94);font:700 14px/1 var(--us-font,Manrope,"Segoe UI",sans-serif);font-variant-numeric:tabular-nums}
#${ROOT_ID}.jt-collapsed header>[data-role="compact-time"]{padding-left:12px;border-left:1px solid rgba(255,255,255,.07)}
#${ROOT_ID} header>button{width:33px;height:33px;padding:0;border:1px solid rgba(255,255,255,.075);border-radius:9px;background:rgba(255,255,255,.035);color:rgba(235,239,244,.76);cursor:pointer;box-shadow:none}
#${ROOT_ID}.jt-collapsed header>button{width:34px;height:34px;border-radius:11px;background:rgba(255,255,255,.045)}
#${ROOT_ID} header>button:hover{background:rgba(255,255,255,.075);border-color:rgba(255,255,255,.11);color:#fff}
#${ROOT_ID} section{padding:10px}
#${ROOT_ID}.jt-collapsed section{display:none}
...
#${ROOT_ID} .jt-tabs::-webkit-scrollbar{display:none}
#${ROOT_ID} .jt-tab{
  --tc:123,170,242;
  position:relative;
  flex:0 0 auto;
  ...
  border:1px solid rgba(var(--tc),.20);
  border-bottom-color:rgba(255,255,255,.055);
  border-radius:13px 13px 0 0;
  background:linear-gradient(180deg,rgba(var(--tc),.075),rgba(var(--tc),.016)),rgba(18,19,24,.74);
  color:rgba(var(--tc),.92);
  box-shadow:0 5px 16px rgba(0,0,0,.12),inset 0 1px 0 rgba(255,255,255,.02);
  -webkit-backdrop-filter:blur(16px) saturate(114%) brightness(90%);
  backdrop-filter:blur(16px) saturate(114%) brightness(90%);
  cursor:grab;
  user-select:none;
  transition:transform 130ms ease,opacity 130ms ease,border-color 130ms ease,background-color 130ms ease;
}
#${ROOT_ID} .jt-tab:hover{background:linear-gradient(180deg,rgba(var(--tc),.11),rgba(var(--tc),.022)),rgba(19,20,26,.82);border-color:rgba(var(--tc),.28)}
#${ROOT_ID} .jt-tab.jt-selected{z-index:4;height:40px;transform:translateY(1px);color:#f8fafc;background:linear-gradient(180deg,rgba(var(--tc),.10),rgba(var(--tc),.022)),rgba(20,21,27,.88);border-color:rgba(var(--tc),.30);border-bottom-color:rgba(16,17,21,.92);box-shadow:0 -5px 14px rgba(0,0,0,.14),inset 0 1px 0 rgba(255,255,255,.03)}
#${ROOT_ID} .jt-tab.jt-dragging{opacity:.42;transform:translateY(2px) scale(.985);box-shadow:none}
#${ROOT_ID} .jt-tab.jt-drop-before::after,#${ROOT_ID} .jt-tab.jt-drop-after::after{content:"";position:absolute;top:6px;bottom:5px;width:2px;border-radius:999px;background:rgb(var(--tc));box-shadow:0 0 0 3px rgba(var(--tc),.10)}
#${ROOT_ID} .jt-tab.jt-drop-before::after{left:-3px}
#${ROOT_ID} .jt-tab.jt-drop-after::after{right:-3px}
#${ROOT_ID} .jt-tab-level-0{--tc:123,170,242}
#${ROOT_ID} .jt-tab-level-1{--tc:217,185,105}
#${ROOT_ID} .jt-tab-level-2{--tc:226,146,85}
#${ROOT_ID} .jt-tab-level-3{--tc:221,105,105}
#${ROOT_ID} .jt-tab-dot{width:6px;height:6px;flex:0 0 6px;border-radius:50%;background:rgb(var(--tc));box-shadow:0 0 0 3px rgba(var(--tc),.085)}
#${ROOT_ID} .jt-tab>span{min-width:0;max-width:74px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:650;color:inherit}
#${ROOT_ID} .jt-tab-time{margin-left:auto;white-space:nowrap;color:rgba(var(--tc),.82);font:650 10px/1 var(--us-font,Manrope,"Segoe UI",sans-serif);font-variant-numeric:tabular-nums}
#${ROOT_ID} .jt-x{
  width:18px;
  height:18px;
  min-width:18px;
  ...
  background-image:
    linear-gradient(45deg,transparent calc(50% - .8px),rgba(236,240,245,.72) calc(50% - .8px),rgba(236,240,245,.72) calc(50% + .8px),transparent calc(50% + .8px)),
    linear-gradient(-45deg,transparent calc(50% - .8px),rgba(236,240,245,.72) calc(50% - .8px),rgba(236,240,245,.72) calc(50% + .8px),transparent calc(50% + .8px));
  background-size:9px 9px,9px 9px;
  background-position:center,center;
  background-repeat:no-repeat;
}
#${ROOT_ID} .jt-x:hover{background-color:rgba(255,255,255,.10)}

#${ROOT_ID} .jt-main,#${ROOT_ID} .jt-empty{border:1px solid rgba(255,255,255,.065);border-radius:12px;background:rgba(255,255,255,.016);box-shadow:none}
#${ROOT_ID} .jt-main{padding:14px 14px 13px}
#${ROOT_ID} .jt-main-head{display:flex;justify-content:space-between;gap:12px}
#${ROOT_ID} .jt-main-head>div{min-width:0}
#${ROOT_ID} .jt-main-head b{display:block;color:rgba(242,245,248,.92);font-size:12.5px;font-weight:650}
#${ROOT_ID} .jt-main-head span{display:block;max-width:330px;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:rgba(210,216,224,.63);font-size:11.5px}
#${ROOT_ID} .jt-main-head em{flex:0 0 auto;padding:5px 8px;border:1px solid rgba(255,255,255,.065);border-radius:8px;background:rgba(255,255,255,.025);color:rgba(215,221,229,.64);font-size:9px;font-style:normal;font-weight:700;text-transform:uppercase;letter-spacing:.02em}
#${ROOT_ID} .jt-main-head em[data-timer-state="running"]{border-color:rgba(80,151,205,.20);background:rgba(80,151,205,.09);color:rgba(184,217,242,.88)}
#${ROOT_ID} .jt-main-head em[data-timer-state="resume"]{border-color:rgba(217,185,105,.18);background:rgba(217,185,105,.07);color:rgba(232,207,149,.88)}
#${ROOT_ID} .jt-main>strong{display:block;margin-top:11px;color:rgba(247,249,251,.96);font-size:34px;font-weight:650;line-height:1;letter-spacing:-.035em;font-variant-numeric:tabular-nums}
#${ROOT_ID} .jt-empty{padding:12px;color:rgba(205,211,220,.62)}

#${ROOT_ID} .jt-resume{margin-bottom:8px;padding:9px 10px;border:1px solid rgba(217,185,105,.14);border-radius:10px;background:rgba(217,185,105,.05)}
#${ROOT_ID} .jt-resume>b{display:block;color:rgba(246,240,226,.94);font-size:11.5px}
#${ROOT_ID} .jt-resume>span{display:block;margin-top:2px;color:rgba(219,209,187,.62);font-size:10.5px}
#${ROOT_ID} .jt-resume>div{display:flex;gap:7px;margin-top:9px}

#${ROOT_ID} .jt-settings{display:none;margin-top:9px;overflow:hidden;border:1px solid rgba(255,255,255,.065);border-radius:12px;background:var(--jt-glass-soft);box-shadow:none}
#${ROOT_ID}.jt-settings-open .jt-settings{display:flex;flex-direction:column}
#${ROOT_ID} .jt-settings>h4{margin:0;padding:11px 11px 7px;color:rgba(239,242,246,.90);font-size:11.5px;font-weight:650}
#${ROOT_ID} .jt-thresholds{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;padding:0 11px 11px}
#${ROOT_ID} .jt-thresholds label{color:rgba(207,213,221,.60);font-size:9.5px;font-weight:550}
#${ROOT_ID} .jt-thresholds input{display:block;width:100%;height:32px;margin-top:4px;padding:5px 8px;border:1px solid rgba(255,255,255,.07);border-radius:8px;background:rgba(255,255,255,.025);color:rgba(240,243,247,.90);box-shadow:none}
#${ROOT_ID} .jt-thresholds input:focus{border-color:rgba(103,169,218,.28);box-shadow:0 0 0 3px rgba(103,169,218,.055);outline:none}
#${ROOT_ID} .jt-archive-scroll{min-height:92px;max-height:min(360px,43vh);overflow-y:auto;overscroll-behavior:contain;padding:4px 10px 10px;border-top:1px solid rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.05);scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.16) transparent}
...
#${ROOT_ID} .jt-archive-section h5{position:sticky;top:-4px;z-index:2;margin:0 -2px 6px;padding:8px 2px 6px;background:linear-gradient(180deg,rgba(14,14,17,.96) 70%,rgba(14,14,17,.72));color:rgba(223,228,234,.68);font-size:9px;font-weight:700;letter-spacing:.065em;text-transform:uppercase}
#${ROOT_ID} .jt-archive-empty{display:block;padding:6px 2px;color:rgba(198,205,214,.47);font-size:9.5px}
#${ROOT_ID} .jt-row{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:7px;margin-top:5px;padding:7px 8px;border:1px solid rgba(255,255,255,.05);border-radius:8px;background:rgba(255,255,255,.015)}
#${ROOT_ID} .jt-row b{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:rgba(229,233,238,.82);font-size:10.5px;font-weight:600}
#${ROOT_ID} .jt-row small{display:block;margin-top:2px;color:rgba(198,205,214,.47);font-size:9.5px}
#${ROOT_ID} .jt-row code{color:rgba(224,229,235,.68);font:10.5px/1.2 ui-monospace,monospace}
#${ROOT_ID} .jt-actions{display:flex;gap:7px;flex-wrap:wrap;padding:9px 11px 11px}
#${ROOT_ID} .jt-btn{min-height:31px;padding:6px 10px;border:1px solid rgba(255,255,255,.07);border-radius:8px;background:rgba(255,255,255,.035);color:rgba(237,240,244,.84);font-size:10.5px;font-weight:600;cursor:pointer;box-shadow:none}
#${ROOT_ID} .jt-btn:hover{background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.10);color:#fff}
#${ROOT_ID} .jt-primary{background:rgba(123,170,242,.10);border-color:rgba(123,170,242,.20)}
#${ROOT_ID} .jt-danger{border-color:rgba(206,115,115,.12);background:rgba(206,115,115,.035);color:rgba(237,196,196,.78)}

#${ROOT_ID}.jt-level-0{--a:#7baaf2;--as:rgba(123,170,242,.14)}
#${ROOT_ID}.jt-level-1{--a:#d9b969;--as:rgba(217,185,105,.14)}
#${ROOT_ID}.jt-level-2{--a:#e29255;--as:rgba(226,146,85,.14)}
#${ROOT_ID}.jt-level-3{--a:#dd6969;--as:rgba(221,105,105,.15)}

@media(max-width:640px){
  #${ROOT_ID}{right:8px;bottom:8px;width:calc(100vw - 16px);padding-top:37px}
  #${ROOT_ID} .jt-tabs{left:9px;right:9px;top:-37px;height:38px}
  #${ROOT_ID} .jt-tab{min-width:88px;max-width:155px;height:35px}
  #${ROOT_ID} .jt-tab.jt-selected{height:38px}
  #${ROOT_ID} .jt-tab-time{font-size:9.5px}
  #${ROOT_ID} .jt-main>strong{font-size:31px}
  #${ROOT_ID} .jt-thresholds{grid-template-columns:1fr}
  #${ROOT_ID}.jt-collapsed .jt-brand>span{display:none}
